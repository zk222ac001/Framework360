...
function normalizeDemoRequestInput(body) {
  return {
    email: normalizeEmail(body.email),
    firstName: normalizeName(body.firstName),
    lastName: normalizeName(body.lastName),
    companyName: normalizeCompanyName(body.companyName),
    jobTitle: normalizeNullableString(body.jobTitle),
    country: normalizeNullableString(body.country),
  };
}

async function ensureDemoCompany(demoRequest) {
  if (demoRequest.company) {
    if (!demoRequest.company.subscriptionStatus || !demoRequest.company.subscriptionRenewal) {
      return prisma.company.update({
        where: { id: demoRequest.company.id },
        data: createTrialSubscriptionData(),
      });
    }

    return demoRequest.company;
  }
...
router.post("/", validate(createDemoRequestSchema), async (req, res) => {
  try {
    const normalized = normalizeDemoRequestInput(req.body);

    if (!isCompanyEmail(normalized.email)) {
      return res.status(400).json({ error: "Please use your company email address" });
    }

    const [registeredUser, existing] = await Promise.all([
      prisma.user.findUnique({ where: { email: normalized.email }, select: { id: true } }),
    ...
    if (existing) {
      return res.status(409).json({ error: "An active demo request already exists for this email" });
    }
...
router.get("/", requireAuth, requirePlatformAdmin, async (req, res) => {
  try {
    const demoRequests = await prisma.demoRequest.findMany({
      ...
      include: { company: true, createdUser: true },
    });
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (!current) return res.status(404).json({ error: "Demo request not found" });
...
router.delete("/:id", requireAuth, requirePlatformAdmin, async (req, res) => {
  try {
    ...
    if (!demoRequest) return res.status(404).json({ error: "Demo request not found" });
    ...
    return res.json({ deleted: true, deletedUser: Boolean(demoRequest.createdUserId) });
  } catch (error) {
...
router.post("/:id/activate", requireAuth, requirePlatformAdmin, async (req, res) => {
  try {
    ...
    const demoRequest = await prisma.demoRequest.findUnique({
      where: { id },
      include: { createdUser: true, company: true },
    });
    ...
    const company = await ensureDemoCompany(demoRequest);
    ...
    return res.status(201).json({
      ...
      temporaryPassword,
      message: "Demo request activated with a trial subscription. Copy this temporary password now; it will not be shown again. Tell the user to change it immediately after login or use Forgot password.",
    });
  } catch (error) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const allowedStatuses = ["PENDING", "EMAILED", "ACTIVATED", "EXPIRED", "REJECTED"];
